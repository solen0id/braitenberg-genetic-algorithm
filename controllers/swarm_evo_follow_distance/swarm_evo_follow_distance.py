...
from neural import FollowLightDistanceFitness, NeuralNetwork
...
def evaluate_fitness():
    ...
    # euclidean distance from starting position is l2 norm
    fitness.distance_from_start += np.linalg.norm(
        np.array([x, y]) - np.array([fitness.x_start, fitness.y_start])
    )
...
def init_robot():
    ...
    if frame_counter == 1:
        ...
        fitness = FollowLightDistanceFitness(
            light_intensity=ll + lr,
            x_start=x,
            y_start=y,
        )
